sap.ui.define([], function () {
  ...
  return {
          ...
          success: (res) => {
            try {
              ...
              if (typeof res === 'string') {
                const hasInvalidChars = /[\u0000-\u001F\u007F-\u009F]/.test(res);
                if (hasInvalidChars) {
                  let cleanedResponse = res.replace(/[\u0000-\u001F\u007F-\u009F]/g, '');
    ...
    uploadTemplate: async function({DOKAR,DATA_INI,DATA_FINE,FILE64,EXT,DATA_CREA}){
      return await this.createEntity({ 
        entity: "/upload_file_template.json",
        data: { DOKAR, DATA_INI,DATA_FINE,FILE64,EXT,DATA_CREA}
      })
    },
    uploadDms: async function({DOKAR,DOKNR,DOKVR,DOKTL,DOKST,OBJKY,DATA_INI,DATA_FINE,FILE64,EXT}){
      return await this.createEntity({ 
        entity: "/upload_allegatidms.json",
        data: { DOKAR,DOKNR,DOKVR,DOKTL,DOKST,OBJKY,DATA_INI,DATA_FINE,FILE64,EXT}
      })
    },
    ...
    getListaRdo: async function({EINDT_DA,EINDT_A,MATNR,LIFNR,STATO,EBELN}){
      return await this.createEntity({ 
    ...
    getDetailAllegato:async function({GUID,DOKAR,DOKNR,DOKVR,DOKTL,TIPO}){
      return await this.createEntity({ 
    ...
    uploadAllPos: async function({DOKAR,DOKNR,DOKVR,DOKTL,DOKST,EBELN,EBELP,FILE64,EXT,DOCFILE}){
      return await this.createEntity({ 
        entity: "/upload_all_pos.json",
        data:{DOKAR,DOKNR,DOKVR,DOKTL,DOKST,EBELN,EBELP,FILE64,EXT,DOCFILE}
      })
    },
    ...
    getListaOda: async function({EINDT_DA,EINDT_A,MATNR,LIFNR,STATO,EBELN}){
      return await this.createEntity({ 
